...
export function CommunityOnlyToggle({ active }: { active: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <Switch id="community-only" checked={active} onCheckedChange={toggle} />
      <Label htmlFor="community-only" className="text-sm font-normal text-muted-foreground">
        Trialled by community
      </Label>
    </div>
  );
}
